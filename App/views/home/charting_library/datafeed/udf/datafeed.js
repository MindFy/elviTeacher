...
Datafeeds.UDFCompatibleDatafeed.prototype._initialize = function () {
  ...
  this._send(`${this._datafeedURL}/config`)
    .done((response) => {
      const configurationData = parseJSONorNot(response)
      configurationData.symbolsTypes = [
        { name: '全部商品', value: '' },
        { name: '股票', value: 'stock' },
        { name: '股指', value: 'index' },
        { name: '差价合约(CFD)', value: 'cfdindice' },
        { name: '债券', value: 'bond' },
        { name: '现货', value: 'commodity' },
        { name: '外汇', value: 'forex' },
      ]
      /* configurationData.supportedResolutions = configurationData.supportedResolutions.filter(function (i) {
				return i != 5 && i != 15 && i != 30 && i != 60 && i != 240;
			}); */
...
Datafeeds.UDFCompatibleDatafeed.prototype.searchSymbols = function (searchString, exchange, type, onResultReadyCallback) {
  ...
  if (this._configuration.supports_search) {
    this._send(`${this._datafeedURL}/search`, {
      ...
      .done((response) => {
        const data = parseJSONorNot(response)
...
Datafeeds.UDFCompatibleDatafeed.prototype.resolveSymbol = function (symbolName, onSymbolResolvedCallback, onResolveErrorCallback) {
  ...
  function onResultReady(data) {
    if (data.type === 'stock') {
      data.session = '0900-1500'
    }
    data.ticker = data.name.toUpperCase()
    data.intraday_multipliers = ['1']
...
Datafeeds.UDFCompatibleDatafeed.prototype.getBars = function (symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback) {
  ...
  /** ***********************just fetch last one years data in D,W,M period***************************** */
  ...
  switch (`${resolution}`) {
    ...
    case 'D':
      resolution = '1d'
      rangeStartDate = rangeEndDate - 365 * 24 * 60 * 60
      break
    case 'W':
      resolution = '1w'
      rangeStartDate = rangeEndDate - 365 * 24 * 60 * 60
  ...
  this._send(this._datafeedURL + this._historyURL, {
    symbol: symbolInfo.name, // ticker.toUpperCase()
    resolution,
    from: rangeStartDate,
    to: rangeEndDate,
  })
    .done((response) => {
      ...
      for (let i = 0; i < barsCount; ++i) {
        ...
        if (window.tvWidget.chart().symbolExt().type === 'stock') {
          barValue.time -= 28800 * 1000
        }
...
Datafeeds.SymbolSearchComponent.prototype.searchSymbols = function (searchArgument, maxSearchResults) {
  ...
  const results = [] // array of WeightedItem { item, weight }
  const queryIsEmpty = !searchArgument.searchString || searchArgument.searchString.length === 0
  const searchStringUpperCase = searchArgument.searchString.toUpperCase()
  ...
  for (let i = 0; i < symbolsStorage._symbolsList.length; ++i) {
    ...
    const positionInName = item.name.toUpperCase().indexOf(searchStringUpperCase)
    const positionInDescription = item.description.toUpperCase().indexOf(searchStringUpperCase)
  ...
  searchArgument.onResultReadyCallback(
    results
      .sort((weightedItem1, weightedItem2) => weightedItem1.weight - weightedItem2.weight)
      .map((weightedItem) => {
        const item = weightedItem.item
        return {
          symbol: item.name,
...
Datafeeds.QuotesPulseUpdater = function (datafeed) {
  ...
  setInterval(() => {
    that._updateQuotes(subscriptionRecord => subscriptionRecord.symbols)
  }, this._updateInterval)

  setInterval(() => {
    that._updateQuotes(subscriptionRecord => (subscriptionRecord.fastSymbols.length > 0 ? subscriptionRecord.fastSymbols : subscriptionRecord.symbols))
